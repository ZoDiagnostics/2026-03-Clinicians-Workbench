...
const AddEditStaffModal: React.FC<AddEditStaffModalProps> = ({ user, clinics, practiceId, onClose }) => {
  const [formData, setFormData] = useState({
    ...
    email: user?.email || '',
    role: user?.role || UserRole.ClinicalStaff,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (user) {
      ...
      await createUser({ practiceId, ...formData });
    }
    onClose();
  };
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="mt-6 grid grid-cols-1 gap-y-6 gap-x-4 sm:grid-cols-6">
                        ...
                        <div className="sm:col-span-6">
                            ...
                            <input type="email" name="email" id="email" value={formData.email} onChange={handleInputChange} disabled={!!user} className="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" />
                        </div>
                        <div className="sm:col-span-6">
